...
#error "OpenSSL 1.1.0 or newer is required to build this program"
#endif
#include <iostream>
...
template<typename T> 
class hash_generator__: public T {      /* generates the hash */
    const EVP_MD *md;
    EVP_MD_CTX *mdctx;                  /* opaque in OpenSSL 1.1+ */
    bool initialized;
    bool finalized;
...
public:
    ...
    void update(const uint8_t *buf, size_t bufsize) {
        if (!initialized)
            init();
        if (finalized) {
            std::cerr << "hash_generator__::update called after final()\n";
            std::abort();
        }
        if (EVP_DigestUpdate(mdctx, buf, bufsize) != 1) {
            std::cerr << "EVP_DigestUpdate failed\n";
            std::abort();
        }
        hashed_bytes += static_cast<int64_t>(bufsize);
    }
    ...
    hash__<T> final() {
        ...
        hash__<T> val;
        unsigned int len = static_cast<unsigned int>(sizeof(val.digest));

        if (EVP_DigestFinal_ex(mdctx, val.digest, &len) != 1) {
